...
extern PCD_HandleTypeDef hpcd_USB_DRD_FS;   /* defined in Core/Src/main.c */
...
/* HAL_PCD_MspInit/DeInit live in Core/Src/stm32g0xx_hal_msp.c (CubeMX);
 * the USB interrupt is enabled there via a USER CODE block. */
...
#endif /* USE_HAL_PCD_REGISTER_CALLBACKS */
{
  ...
  /* Always full speed on this part. (Note: CubeMX writes PCD_SPEED_FULL=2
   * into Init.speed while the LL header calls FS 1 - don't compare them.) */
...
{
  /* The PCD itself is already initialized by CubeMX's
   * MX_USB_DRD_FS_PCD_Init(); only link it to the device library here. */
...
/* USB global interrupt (shared with UCPD1/2 on G0B1) */
